...
class RegisterController extends Controller
{
	...
	public function register(Request $req)
	{
		$valid = $req->validate([
			...
			'registration_username' => 'required|regex:/(^[A-Za-z0-9 ]+$)+/|min:2|max:15',
			'registration_firstname' => 'required|regex:/(^[A-Za-z0-9 ]+$)+/|min:2|max:15',
			'registration_lastname' => 'required|regex:/(^[A-Za-z0-9 ]+$)+/|min:2|max:15'
		]);
		...
		Bill::events()->create('global', false, 'registration', $req->input('registration_username').' Just registered to the panel');
		return view('templates/auth.register', [
			'locations' => $this->repository->getAllWithDetails(),
			'post_res' => $res,
			'settings' => BillingSettings::getAll(),
